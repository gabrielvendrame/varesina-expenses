...
export function CurrencyComboBox() {
    ...
    const mutation = useMutation({
        ...
        onSuccess: (data: UserSettings) => {
            toast.success("Valuta aggiornata", {id: "update-currency"})
            setSelectedOption(Currencies.find(currency => currency.value === data.currency) || null)
        },
        onError: (e) => {
            console.error(e)
            toast.error("Errore durante l'aggiornamento della valuta", {id: "update-currency"})
        }
    })
    ...
    const selectOption = useCallback((currency: Currency | null) => {
        ...
        toast.loading("Aggiornamento valuta...", {id: "update-currency"})
                ...
                <Popover open={open} onOpenChange={setOpen}>
                    <PopoverTrigger asChild>
                        <Button variant="outline" className="w-full justify-start" disabled={mutation.isPending}>
                            {selectedOption ? <>{selectedOption.label}</> : <>+ Seleziona una valuta</>}
                        </Button>
                    </PopoverTrigger>
